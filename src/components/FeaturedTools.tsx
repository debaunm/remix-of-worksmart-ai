...
import { PenLine, MessageSquare, Target, ClipboardList, Lightbulb, User, Presentation, TrendingUp, FileText, Sparkles, Utensils, PenTool } from "lucide-react";
...
const FeaturedTools = () => {
  ...
  return <section id="features" className="py-24 bg-secondary/50">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          length: 6
        }).map((_, index) => <div key={index} className="rounded-2xl bg-white border border-border p-6 shadow-card">
          ...
          return <motion.div key={tool.id} initial={{
            opacity: 0,
            y: 20
          }} whileInView={{
            opacity: 1,
                  ...
                  <Link to={tool.route || "#"} className="block h-full">
                    <div className="h-full rounded-2xl bg-white border border-border p-6 hover:border-primary/50 hover:shadow-card-hover transition-all duration-300 cursor-pointer group">
                      ...
                      <div className="flex items-start justify-between mb-4 text-accent-foreground">
                        ...
                        {tool.badge && <span className="text-xs font-medium bg-primary/10 text-primary px-2 py-1 rounded-full">
                            {tool.badge}
                          </span>}
                      </div>
